...
function Progress({
  ...
  return (
    <ProgressPrimitive.Root
      ...
      {...props}
    >
      {value !== 0 && <Indicator value={value} className={`rounded-sm ${indicatorClassName}`} />}
      {value !== 0 && value !== 100 && <Indicator value={97 - (value ?? 0)} className="absolute right-0 bg-muted rounded-sm" />}
      {value === 0 && <Indicator value={100} className="absolute right-0 bg-muted rounded-sm" />}
    </ProgressPrimitive.Root>
  );
}
